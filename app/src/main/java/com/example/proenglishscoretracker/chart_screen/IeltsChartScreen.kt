...
@Composable
fun IeltsChartScreen(viewModel: EnglishInfoViewModel) {
    val ieltsInfoList by viewModel.ieltsInfo.collectAsState()
    val latestExamYear = ieltsInfoList
        .mapNotNull { it.date.substring(0, 4).toIntOrNull() }
        .maxOrNull()
    var examYear by remember { mutableStateOf<Int?>(null) }
...
private fun IeltsScoreChart(
    ...
    val ieltsInfoList by viewModel.ieltsInfo.collectAsState()
    var isGraphTapped by remember { mutableStateOf(false) }

    if (ieltsInfoList.isEmpty()) {
        Box(
        ...
        val filteredIeltsInfo = ieltsInfoList.filter {
            it.date.substring(0, 4).toInt() == examYear
        }

        if (filteredIeltsInfo.isEmpty()) {
            Box(
                modifier = Modifier.fillMaxSize(),
                contentAlignment = Alignment.Center
            ) {
            ...
            val sortedIeltsInfo = filteredIeltsInfo.sortedBy { it.date }
            val examDates = sortedIeltsInfo.map { it.date }
            val overallScores = sortedIeltsInfo.map { it.overallScore.toFloat() }
            val readingScores = sortedIeltsInfo.map { it.readingScore.toFloat() }
            val listeningScores = sortedIeltsInfo.map { it.listeningScore.toFloat() }
            val writingScores = sortedIeltsInfo.map { it.writingScore.toFloat() }
            val speakingScores = sortedIeltsInfo.map { it.speakingScore.toFloat() }
